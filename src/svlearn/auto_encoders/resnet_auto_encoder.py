...
'''
The Resnet Antoencoder class inspired from Resnet50 (from the [link](https://github.com/Horizon2333/imagenet-autoencoder/blob/main/models/resnet.py))
'''
class ResNetAutoEncoder(Autoencoder):

    def __init__(self, configs = configs, bottleneck = bottleneck):
        """ResnetAutoEncoder init takes in configs and bottleneck parameter and overrides the encoder/decoder with Resnet architecture

        Args:
            configs (_type_, optional): _description_. Defaults to configs.
            bottleneck (_type_, optional): _description_. Defaults to bottleneck.
        """
...
class ResNetDecoder(nn.Module):
    ...
    def forward(self, x):
        if len(x.size()) == 2:
            ...
            im_length = int(np.sqrt(flat_dim/num_hidden_channels))
            im_height = im_length
            x = x.view(x.size(0), num_hidden_channels, im_length, im_height)
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
        x = self.conv4(x)
